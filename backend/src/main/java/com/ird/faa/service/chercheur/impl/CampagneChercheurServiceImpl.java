...
import com.ird.faa.service.chercheur.facade.TemplateClotureChercheurService;
...
import com.ird.faa.service.chercheur.facade.CampagneChercheurChercheurService;
...
public class CampagneChercheurServiceImpl extends AbstractServiceImpl<Campagne> implements CampagneChercheurService {
	...
	private void updateAssociatedLists(Campagne campagne) {
		if (campagne == null || campagne.getId() == null)
			...
			/*
			 * List<List<CampagneChercheur>> resultCampagneChercheurs=
			 * campagneChercheurService.getToBeSavedAndToBeDeleted(campagneChercheurService.
			 * findByCampagneId(campagne.getId()),campagne.getCampagneChercheurs());
			...
			List<List<CampagneRelance>> resultCampagneRelances = campagneRelanceService.getToBeSavedAndToBeDeleted(
					campagneRelanceService.findByCampagneId(campagne.getId()), campagne.getCampagneRelances());
	...
	public Campagne findProgressCampagneByChercheurUsername(String username) {
		...
		query += SearchUtil.addConstraint("o", "etatCampagne.code", "!=", Etat.TERMINE);
		...
		if (campagnes != null && !campagnes.isEmpty()) {	
			Campagne campagneFounded= campagnes.get(0);
	        int result = campagneFounded.getDateFin().compareTo(new Date());
	        if (result  > 0) {
	            System.out.println("Date1 is equal to Date2");
	            return campagneFounded;
	        } 
	        else return null;	
		}
		return null;
